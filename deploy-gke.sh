...
# SLAM Robotics GKE Deployment Script
# Complete deployment automation for GKE (Cost-optimized)
...
# Function to create GKE cluster (Cost-optimized)
create_cluster() {
    print_status "Creating cost-optimized GKE cluster..."
    ...
    if gcloud container clusters describe $CLUSTER_NAME --zone=$ZONE &> /dev/null; then
        ...
        print_success "Cost-optimized GKE cluster created"
    fi
...
build_images() {
    ...
    cd backend
    docker build -t gcr.io/$PROJECT_ID/slam-backend:latest .
...
setup_domain() {
    ...
    # Reserve static IP
    gcloud compute addresses create slam-ip --global || true
    
    # Get static IP
    STATIC_IP=$(gcloud compute addresses describe slam-ip --global --format="value(address)")
    
    print_success "Static IP reserved: $STATIC_IP"
    print_warning "Please update your DNS records:"
    echo "Type: A, Name: @, Value: $STATIC_IP"
    echo "Type: A, Name: www, Value: $STATIC_IP"
...
get_deployment_info() {
    ...
    # Get external IPs
    FRONTEND_IP=$(kubectl get service slam-frontend-loadbalancer -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "Not available")
    BACKEND_IP=$(kubectl get service slam-backend-loadbalancer -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "Not available")
    
    echo ""
    echo "=========================================="
    echo "🚀 SLAM ROBOTICS GKE DEPLOYMENT COMPLETED!"
    echo "=========================================="
    echo ""
    ...
    echo "- Machine type: e2-micro (cheapest)"
    echo "- Nodes: 2 (min: 1, max: 5)"
    ...
    echo "2. Update DNS records with the static IP"
    echo "3. Test your application"
    echo "4. Monitor costs in Cloud Console"
    echo ""
}

# Main deployment function
main() {
    echo "🚀 SLAM Robotics GKE Deployment (Cost-Optimized)"
    echo "================================================"
    echo ""
    ...
    print_success "Cost-optimized GKE deployment completed successfully!"
}
